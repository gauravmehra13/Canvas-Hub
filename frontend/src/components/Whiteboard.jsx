...
import { Pen, Eraser, Circle as CircleIcon, Square, ArrowUpRight, Undo2, Redo2, Trash2, PaintBucket } from 'lucide-react';
...
const Whiteboard = ({ roomId, sendDrawing }) => {
  ...
  // Handle received drawings from other users
  useEffect(() => {
  ...
  const handleMouseDown = (e) => {
    ...
    if (tool === TOOLS.PEN || tool === TOOLS.ERASER) {
      const newLine = {
        ...
        lineCap: 'round',
        lineJoin: 'round',
        globalCompositeOperation: tool === TOOLS.ERASER ? 'destination-out' : 'source-over'
      };
      setLines([...lines, newLine]);
    } else {
      const newTempShape = {
        type: tool === TOOLS.CIRCLE ? 'circle' : tool === TOOLS.ARROW ? 'arrow' : 'rect',
        x: pos.x,
        y: pos.y,
        width: 0,
        height: 0,
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-gray-800" ref={containerRef}>
      <div className="border-b border-gray-200 dark:border-gray-700">
        <div className="p-2 flex items-center gap-2">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <span className="text-sm text-gray-600 dark:text-gray-300 min-w-[3ch]">{brushSize}</span>
          </div>
            ...
            {colors.map((c) => (
              <button
                ...
                className={`w-6 h-6 rounded-lg transition-all duration-200 hover:scale-110 ${
                  color === c ? 'ring-2 ring-blue-500 ring-offset-2 dark:ring-offset-gray-800 scale-110' : ''
                }`}
